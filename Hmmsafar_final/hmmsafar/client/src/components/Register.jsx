...
const Register = () => {
  ...
  const { name, email, password, cf_password, pic , destination} = userData;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log(name, email, password, destination ,pic);
      const { data } = await axios.post(
        "/api/users",
        { name, email, password, pic ,destination },
        config
      );
  ...
  return (
    <VStack spacing="30px">
      ...
      <FormControl id="destination" isRequired>
        <FormLabel mb="0px">Destination</FormLabel>
        <Input
          onChange={handleChange}
          name="destination"
          value={destination}
